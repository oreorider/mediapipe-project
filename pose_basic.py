...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence = pose_confidence_value, enable_segmentation = True) as pose:
    ...
    if True:
        ...
        if(config.make_excel):
            result_name = 'pose_basic_result_{}.xlsx'.format(video_name)
            coordinate_name = 'pose_basic_coordinate_{}.xlsx'.format(video_name)
            angle_name = 'pose_basic_body_angle_{}.xlsx'.format(video_name)
